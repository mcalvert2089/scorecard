...
import { saveSinglePlayer, updatePlayerInfo } from '../../../js/actions/index'
...
class PlayersEdit extends React.Component {
	...
	handleSubmit(event) {
	    ...
	    axios.patch('/api/players/' + this.state.id, { name, manager, city, state })
	      .then((result) => {
	        if(result.status === 200) {
	      		store.dispatch( saveSinglePlayer({ team: {
	        			id: result.data.id,
	        			name: result.data.name,
			      		manager: result.data.manager,
			      		city: result.data.city,
